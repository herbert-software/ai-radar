...
function makeStub(opts: {
  ...
  // 桩无法解析消费者反查 plan 的 SQL 片段（`mrPlans.vendorId = vendor.id`），按 vendor 列表顺序模拟
  // 每次 plan lookup 命中一个 matched vendor。若消费者只命中第一个 vendor，第二个 vendor 的 plan 不会被返回。
  const vendorPlanLookupIds = opts.vendors.map((v) => v.id);
  let planLookupIndex = 0;
        ...
        from(table: unknown) {
          ...
          if (table === schema.mrPlans) {
            return {
              where: async () => {
                const vendorId =
                  vendorPlanLookupIds[Math.min(planLookupIndex, vendorPlanLookupIds.length - 1)];
                planLookupIndex += 1;
                return vendorId === undefined ? [] : (opts.plansByVendor[vendorId] ?? []);
              },
            };
          }
...
describe('runEventReview 候选闭区间 + 排 tombstone + NULL 文本 + 打标不改事实', () => {
  ...
  async function run(
    ...
    plansByVendor: Record<string, Array<{ id: string }>>,
    failPlanId?: string,
    vendorRows: VendorFixture[] = vendors,
  ...
  it('同一事件命中两个被跟踪厂商 → 两个厂商的 plan 都被打标', async () => {
    const multiVendors: VendorFixture[] = [
      { id: 'vendor-acme', normalizedName: 'acme' },
      { id: 'vendor-beta', normalizedName: 'betaai' },
    ];
    const events: EventFixture[] = [
      {
        eventId: 'e-roundup',
        representativeTitle: 'Acme and BetaAI pricing roundup for new model plans',
        summaryZh: null,
        headlineZh: null,
    ...
    const { flagCalls, result } = await run(
      events,
      {
        'vendor-acme': [{ id: 'plan-acme' }],
        'vendor-beta': [{ id: 'plan-beta' }],
      },
      undefined,
      multiVendors,
    );
    ...
    expect(result.matchedEvents).toBe(1);
    expect(result.flaggedPlans).toBe(2);
    expect(flagCalls.map((c) => c.targetId).sort()).toEqual(['plan-acme', 'plan-beta']);
    expect(flagCalls.map((c) => c.reason)).toEqual([
      expect.stringContaining('acme'),
      expect.stringContaining('betaai'),
    ]);
  });
