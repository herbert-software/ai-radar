...
系统必须为每条可处理的 `raw_item` 计算 `dedup_key`，并以 `ai_news_events.dedup_key` 的 `UNIQUE` 约束 + `INSERT ... ON CONFLICT (dedup_key) DO UPDATE` 把同一事件的多条 `raw_item` 塌缩为同一条 `ai_news_events`。`dedup_key` 构造必须遵循 fallback 链：`canonical_url` 存在时 `dedup_key = sha256(canonical_url)`；否则 `dedup_key = sha256(title_hash)`。
...
- **当** 两条 `raw_item` 经规范化得到相同 `canonical_url`（因而相同 `dedup_key`）
